...
def ResnetBlock(x, dim, pad_type, norm_layer, use_dropout=False, scope='resblock'):
    with tf.variable_scope(scope):
        s = x
        
        with tf.variable_scope('res1'):
            x = conv(x, dim, kernel=3, stride=1, pad=1, pad_type=pad_type, scope='conv')
            x = norm_layer(x)
            x = relu(x)
            if use_dropout:
...
def l1_loss(x, y):
    return tf.reduce_mean(tf.reduce_mean(tf.abs(x-y),(1,2,3)))
